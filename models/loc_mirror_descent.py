...
class LOC_MirrorDescent(MirrorDescent):
    ...
    def setup(self):
        ...
        self.show("At the setup we choose: {} stages ({} iterates)".format(self.stages_to_perform, self.budget))
    ...
    def stage(self, i, num):
        ...
        init_md_gamma = get_md_gamma_from_class(self, n_iterations)
        sub_model = MirrorDescent(self.oracle, lambda x: init_md_gamma, self.solution, verbose=False)
        ...
        self.solution = get_averaged_estimates(sub_model, start=n_iterations//2)[:, -1:]
    ...
    def make_pass(self, num=None):
        ...
        if num is not None:
            ...
            while self.iter < self.budget:
                if self.verbose:
                    print('Do localization iteration {}{}/{}{}'.format(mycol.bd, self.iter, self.budget, mycol.e))
                self.stage()

    def calculate_iterations(self):
        bias_part = 8 * self['L'] / self['mu']
        noise_part = 256 * sq(self['sigma'] / (self['mu'] * self['omega']))
